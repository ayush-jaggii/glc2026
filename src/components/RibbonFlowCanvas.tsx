...
export default function RibbonFlowCanvas() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const isMobile = width < 768 || screenAspect < 1.05

      // Dynamic nexus focal point aligned with ribbon intersection in both mobile asset and desktop
      let nexusX: number
      let nexusY: number

      if (isMobile) {
        nexusX = width * 0.435 + (mouseRef.current.x - 0.5) * 15
        nexusY = height * 0.628 + (mouseRef.current.y - 0.5) * 12
      } else if (screenAspect < texAspect) {
        // Match FlowingRibbonCanvas desktop cover projection
      ...
      for (const p of particles) {
        ...
        if (p.stream === 'magenta') {
          ...
          const startX = 0
          const startY = height * (isMobile ? 0.56 : 0.32) + p.offsetY
          const cpX = nexusX * 0.45
          const cpY = height * (isMobile ? 0.64 : 0.58) + Math.sin(time * 0.6 + p.offsetY) * 10 + p.offsetY
          ...
          const endX = width
          const endY = height * (isMobile ? 0.68 : 0.68) + p.offsetY
          const cpX = nexusX + (width - nexusX) * 0.52
          const cpY = height * (isMobile ? 0.62 : 0.52) + Math.cos(time * 0.6 + p.offsetY) * 10 + p.offsetY
