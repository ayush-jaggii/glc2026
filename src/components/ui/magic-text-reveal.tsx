...
export const MagicTextReveal: React.FC<MagicTextRevealProps> = ({
  ...
  fontWeight = 700,
  spread = 30,
  speed = 0.25, // Slower, more cinematic particle movement
  density = 4,
  ...
  const updateParticles = useCallback((
    ...
    // Slower, smooth, magical particle physics
    const FLOAT_RADIUS = spread;
    const RETURN_SPEED = 0.9; // Slower return speed when hovered
    const FLOAT_SPEED = speed;
    const TRANSITION_SPEED = 2.2 * FLOAT_SPEED;
    const NOISE_SCALE = 0.5;
    const CHAOS_FACTOR = 0.8;
    const FADE_SPEED = 2.5; // Gentle, gradual fade out speed

    particles.forEach(particle => {
      if (isHovered) {
        // Slowly return particles to original positions
        ...
        // Gentle particle fade out
        particle.opacity = Math.max(0, particle.opacity - FADE_SPEED * deltaTime);
      } else {
        particle.floatingAngle += deltaTime * particle.floatingSpeed * (0.8 + Math.random() * CHAOS_FACTOR);
        
        const time = Date.now() * 0.0005; // Slower time step
        const uniqueOffset = particle.floatingSpeed * 1000;
        const noiseX = (
          Math.sin(time * particle.floatingSpeed + particle.floatingAngle) * 1.0 +
          Math.sin((time + uniqueOffset) * 0.4) * 0.6 +
          (Math.random() - 0.5) * CHAOS_FACTOR
        ) * NOISE_SCALE;
        const noiseY = (
          Math.cos(time * particle.floatingSpeed + particle.floatingAngle * 1.3) * 0.5 +
          Math.cos((time + uniqueOffset) * 0.4) * 0.3 +
          (Math.random() - 0.5) * CHAOS_FACTOR
        ) * NOISE_SCALE;
        ...
        if (distanceFromOrigin > FLOAT_RADIUS) {
          ...
          const pullBack = (distanceFromOrigin - FLOAT_RADIUS) * 0.05;
        ...
        particle.opacity += opacityDiff * particle.sparkleSpeed * deltaTime * 1.5;
        
        if (Math.abs(opacityDiff) < 0.01) {
          particle.targetOpacity = Math.random() < 0.5 
            ? Math.random() * 0.1 * particle.originalAlpha
            : particle.originalAlpha * 2.5;
          particle.sparkleSpeed = Math.random() * 1.5 + 0.5;
        }
      }
    });
  ...
  useEffect(() => {
    const handleResize = () => {
      if (wrapperRef.current && textDimensions.width && textDimensions.height) {
        ...
        const basePadding = isMobile ? Math.max(fontSize * 0.3, 16) : Math.max(fontSize * 0.4, 24);
        
        const minWidth = Math.max(textDimensions.width + basePadding * 2, isMobile ? 120 : 180);
        const minHeight = Math.max(textDimensions.height + basePadding * 2, isMobile ? 48 : 64);
  ...
  return (
    <div
      ref={wrapperRef}
      className={`relative inline-flex items-center justify-center overflow-hidden rounded-xl transition-all duration-700 ease-out ${className}`}
      style={{
    ...
    >
      <div
        className={`absolute z-10 transition-opacity duration-700 font-bold ${
